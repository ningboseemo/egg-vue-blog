...
export default {
  ...
  mounted () {
    this.md = new markdownIt().use(subscript)
      .use(superscript)
      .use(footnote)
      .use(deflist)
      .use(abbreviation)
    ...
    let outHtml = this.show ? this.md.render(this.prerender(this.content)) : ''
    this.outHtml = this.postrender(outHtml)
    // console.log(outHtml)
